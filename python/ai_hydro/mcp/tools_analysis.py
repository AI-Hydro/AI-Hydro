...
# ============================================================================
# Tool: Hydrological Concept Search (RAG)
# ============================================================================

@mcp.tool()
def query_hydro_concepts(query: str, top_k: int = 3) -> dict:
    """
    Search the AI-Hydro hydrological knowledge base.

    Retrieves definitions, formulas, and context for hydrological
    concepts, CAMELS attribute descriptions, and best practices.
    Use this when you need to understand what a metric means or
    how it is calculated before running analysis tools.

    Parameters
    ----------
    query : str
        Natural language concept query, e.g.:
        'baseflow index', 'aridity index formula',
        'topographic wetness index', 'CAMELS attributes list'
    top_k : int
    ...
    -------
    dict with:
        results : list of {concept, definition, category, formula, references}

    Examples
    --------
    >>> query_hydro_concepts('what is baseflow index')
    >>> query_hydro_concepts('CAMELS hydrological signatures list')
    >>> query_hydro_concepts('aridity index calculation')
    """
    try:
        from ai_hydro.rag import RAGEngine

        engine = RAGEngine()
        raw_results = engine.query(query, top_k)
        ...
        log.warning("RAG concept search failed (optional component): %s", e)
        return {
            "query": query,
            "results": [],
            "n_results": 0,
            "warning": "Knowledge base not available. Install with: pip install 'ai-hydro[rag]'",
        }
