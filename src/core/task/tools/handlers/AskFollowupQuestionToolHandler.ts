...
import { AiHydroAsk, AiHydroAskQuestion } from "@shared/ExtensionMessage"
...
export class AskFollowupQuestionToolHandler implements IToolHandler, IPartialBlockHandler {
	readonly name = AiHydroDefaultTool.ASK
	...
	async handlePartialBlock(block: ToolUse, uiHelpers: StronglyTypedUIHelpers): Promise<void> {
		...
		} satisfies AiHydroAskQuestion

		await uiHelpers.ask("followup" as AiHydroAsk, JSON.stringify(sharedMessage), block.partial).catch(() => {})
	}
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		if (config.autoApprovalSettings.enabled && config.autoApprovalSettings.enableNotifications) {
			showSystemNotification({
				subtitle: "AI Hydro has a question...",
		...
		if (optionsRaw && text && options.includes(text)) {
			...
			const lastFollowupMessage = findLast(aihydroMessages, (m: any) => m.ask === "followup")
			if (lastFollowupMessage) {
				lastFollowupMessage.text = JSON.stringify({
					...sharedMessage,
					selected: text,
				} satisfies AiHydroAskQuestion)
				await config.messageState.saveAiHydroMessagesAndUpdateHistory()
			}
		} else {
