...
export class ToolValidator {
	constructor(private readonly aihydroIgnoreController: AiHydroIgnoreController) {}
	...
	/**
	 * Verifies access is allowed to a given path via .aihydroignore rules.
	 * Callers should pass a repo-relative (workspace-relative) path.
	 */
	checkAiHydroIgnorePath(relPath: string): ValidationResult {
		const accessAllowed = this.aihydroIgnoreController.validateAccess(relPath)
		if (!accessAllowed) {
			return {
				ok: false,
				error: `Access to path '${relPath}' is blocked by .aihydroignore settings.`,
			}
		}
		return { ok: true }
	}
