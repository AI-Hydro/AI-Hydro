...
export class PromptBuilder {
	...
	public static async getToolsPrompts(variant: PromptVariant, context: SystemPromptContext) {
		let resolvedTools: ReturnType<typeof AiHydroToolSet.getTools> = []
		...
		if (variant?.tools?.length) {
			...
			resolvedTools = AiHydroToolSet.getToolsForVariantWithFallback(variant.family, requestedIds)
			...
			resolvedTools = AiHydroToolSet.getTools(variant.family)
	...
	public static tool(config: AiHydroToolSpec, registry: AiHydroDefaultTool[], context: SystemPromptContext): string {
