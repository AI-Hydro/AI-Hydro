 ...
 * Designed to be instantiated once in the task module and passed to file manipulation services.
 * Uses the 'ignore' library to support standard .gitignore syntax in .aihydroignore files.
 */
export class AiHydroIgnoreController {
	private cwd: string
	private ignoreInstance: Ignore
	private fileWatcher?: FSWatcher
	aihydroIgnoreContent: string | undefined

	constructor(cwd: string) {
		this.cwd = cwd
		this.ignoreInstance = ignore()
		this.aihydroIgnoreContent = undefined
	}
	...
	async initialize(): Promise<void> {
		// Set up file watcher for .aihydroignore
		this.setupFileWatcher()
		await this.loadAiHydroIgnore()
	}

	/**
	 * Set up the file watcher for .aihydroignore changes
	 */
	private setupFileWatcher(): void {
		const ignorePath = path.join(this.cwd, ".aihydroignore")
		...
		this.fileWatcher.on("error", (error) => {
			console.error("Error watching .aihydroignore file:", error)
		})
	}

	/**
	 * Load custom patterns from .aihydroignore if it exists.
	 * Supports "!include <filename>" to load additional ignore patterns from other files.
	 */
	private async loadAiHydroIgnore(): Promise<void> {
		try {
			...
			if (await fileExistsAtPath(ignorePath)) {
				...
				this.aihydroIgnoreContent = content
				await this.processIgnoreContent(content)
				this.ignoreInstance.add(".aihydroignore")
			} else {
				this.aihydroIgnoreContent = undefined
			}
		} catch (error) {
			// Should never happen: reading file failed even though it exists
			console.error("Unexpected error loading .aihydroignore:", error)
		}
	}
	...
	private async processIgnoreContent(content: string): Promise<void> {
		...
		const combinedContent = await this.processAiHydroIgnoreIncludes(content)
	 ...
	 */
	private async processAiHydroIgnoreIncludes(content: string): Promise<string> {
		let combinedContent = ""
	...
	private async readIncludedFile(includeLine: string): Promise<string | null> {
		...
		if (!(await fileExistsAtPath(resolvedIncludePath))) {
			console.debug(`[AiHydroIgnore] Included file not found: ${resolvedIncludePath}`)
			return null
		}
	...
	validateAccess(filePath: string): boolean {
		// Always allow access if .aihydroignore does not exist
		if (!this.aihydroIgnoreContent) {
			return true
		}
		try {
	...
	validateCommand(command: string): string | undefined {
		// Always allow if no .aihydroignore exists
		if (!this.aihydroIgnoreContent) {
			return undefined
		}
