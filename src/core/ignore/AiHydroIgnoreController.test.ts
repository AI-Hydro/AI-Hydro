...
import { AiHydroIgnoreController } from "./AiHydroIgnoreController"
import "should"

describe("AiHydroIgnoreController", () => {
	let tempDir: string
	let controller: AiHydroIgnoreController
	...
	beforeEach(async () => {
		...
		// Create default .aihydroignore file
		await fs.writeFile(
			path.join(tempDir, ".aihydroignore"),
			[".env", "*.secret", "private/", "# This is a comment", "", "temp.*", "file-with-space-at-end.* ", "**/.git/**"].join(
				"\n",
			),
		)

		controller = new AiHydroIgnoreController(tempDir)
	...
	describe("Default Patterns", () => {
		...
		it("should block access to .aihydroignore file", async () => {
			const result = controller.validateAccess(".aihydroignore")
	...
	describe("Custom Patterns", () => {
		...
		it("should handle comments in .aihydroignore", async () => {
			// Create a new .aihydroignore with comments
			await fs.writeFile(
				path.join(tempDir, ".aihydroignore"),
	...
	describe("Error Handling", () => {
		...
		it("should handle missing .aihydroignore gracefully", async () => {
			// Create a new controller in a directory without .aihydroignore
			const emptyDir = path.join(os.tmpdir(), `llm-test-empty-${Date.now()}`)
			await fs.mkdir(emptyDir)

			try {
				const controller = new AiHydroIgnoreController(emptyDir)
				await controller.initialize()
		...
		it("should handle empty .aihydroignore", async () => {
			await fs.writeFile(path.join(tempDir, ".aihydroignore"), "")
	...
	describe("Include Directive", () => {
		it("should load patterns from an included file", async () => {
			...
			// Create a .aihydroignore file that includes .gitignore and adds an extra pattern "secret.txt"
			await fs.writeFile(path.join(tempDir, ".aihydroignore"), ["!include .gitignore", "secret.txt"].join("\n"))

			// Initialize the controller to load the updated .aihydroignore
			controller = new AiHydroIgnoreController(tempDir)
			await controller.initialize()
			...
			// "secret.txt" should be ignored as specified directly in .aihydroignore
		...
		it("should handle non-existent included file gracefully", async () => {
			// Create a .aihydroignore file that includes a non-existent file
			await fs.writeFile(path.join(tempDir, ".aihydroignore"), ["!include missing-file.txt"].join("\n"))
		...
		it("should handle non-existent included file gracefully alongside a valid pattern", async () => {
			...
			await fs.writeFile(path.join(tempDir, ".aihydroignore"), ["!include non-existent.txt", "*.tmp"].join("\n"))
