...
describe("Filesystem Utilities", () => {
	const tmpDir = path.join(os.tmpdir(), "aihydro-test-" + Math.random().toString(36).slice(2))
	...
	it("should exclude .aihydrorules/workflows directory specifically", async () => {
		// Create a test directory structure
		const aihydrorulesDirTest = path.join(tmpDir, "aihydrorules-test")
		const aihydrorulesDirPath = path.join(aihydrorulesDirTest, ".aihydrorules")

		// Create .aihydrorules directory and root files
		await fs.mkdir(aihydrorulesDirPath, { recursive: true })
		await fs.writeFile(path.join(aihydrorulesDirPath, "config.json"), "{}")
		await fs.writeFile(path.join(aihydrorulesDirPath, "settings.js"), "// settings")

		// Create .aihydrorules/other directory and files
		const otherDirPath = path.join(aihydrorulesDirPath, "other")
		...
		// Create .aihydrorules/workflows directory and files
		const workflowsDirPath = path.join(aihydrorulesDirPath, "workflows")
		...
		const allFiles = await readDirectory(aihydrorulesDirPath)
		...
		const filteredFiles = await readDirectory(aihydrorulesDirPath, [[".aihydrorules", "workflows"]])

		// Verify workflows files are excluded but others remain
		filteredFiles.length.should.equal(4) // 2 in root + 2 in other

		const expectedFiles = [
			path.resolve(aihydrorulesDirPath, "config.json"),
			path.resolve(aihydrorulesDirPath, "settings.js"),
		...
		const multiExcludeFiles = await readDirectory(aihydrorulesDirPath, [
		...
		const rootOnlyFiles = [path.resolve(aihydrorulesDirPath, "config.json"), path.resolve(aihydrorulesDirPath, "settings.js")]
