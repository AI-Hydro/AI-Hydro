...
function setupHostProvider(extensionContext: any, extensionDir: string, dataDir: string) {
	...
	// aihydro-core expects the binaries to be unpacked in the directory where it is running.
...
/**
 * Gracefully shutdown the aihydro-core process by:
...
function showHelp() {
	console.log(`
AI-Hydro Core - Standalone Server

Usage: node aihydro-core.js [options]
...
Options:
  ...
  -c, --config <path>            Directory for AI-Hydro data storage (default: ~/.aihydro)
