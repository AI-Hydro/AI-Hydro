...
import { AiHydroApiReqInfo, AiHydroMessage, AiHydroSay } from "@shared/ExtensionMessage"
...
export class TaskCheckpointManager implements ICheckpointManager {
	...
	async saveCheckpoint(isAttemptCompletionMessage: boolean = false, completionMessageTs?: number): Promise<void> {
		try {
			...
			const aihydroMessages = this.services.messageStateHandler.getAiHydroMessages()
			aihydroMessages.forEach((message) => {
			...
			if (!isAttemptCompletionMessage) {
				...
				if (messageTs) {
					const messages = this.services.messageStateHandler.getAiHydroMessages()
						...
						this.state.checkpointTracker
							...
							.then(async (commitHash) => {
								if (commitHash) {
									...
									await this.services.messageStateHandler.saveAiHydroMessagesAndUpdateHistory()
								}
							})
							.catch((error) => {
								console.error(
				...
				const lastFiveaihydroMessages = this.services.messageStateHandler.getAiHydroMessages().slice(-3)
				const lastCompletionResultMessage = findLast(lastFiveaihydroMessages, (m) => m.say === "completion_result")
	...
	async restoreCheckpoint(
		messageTs: number,
		restoreType: AiHydroCheckpointRestore,
		offset?: number,
	): Promise<CheckpointRestoreStateUpdate> {
		try {
			const aihydroMessages = this.services.messageStateHandler.getAiHydroMessages()
			const messageIndex = aihydroMessages.findIndex((m) => m.ts === messageTs) - (offset || 0)
			// Find the last message before messageIndex that has a lastCheckpointHash
			const lastHashIndex = findLastIndex(aihydroMessages.slice(0, messageIndex), (m) => m.lastCheckpointHash !== undefined)
			const message = aihydroMessages[messageIndex]
			const lastMessageWithHash = aihydroMessages[lastHashIndex]
	...
	async presentMultifileDiff(messageTs: number, seeNewChangesSinceLastTaskCompletion: boolean): Promise<void> {
		...
		try {
			...
			const aihydroMessages = this.services.messageStateHandler.getAiHydroMessages()
			const messageIndex = aihydroMessages.findIndex((m) => m.ts === messageTs)
			const message = aihydroMessages[messageIndex]
			if (!message) {
				console.error(`[TaskCheckpointManager] Message not found for timestamp ${messageTs} in task ${this.task.taskId}`)
				relinquishButton()
			...
			if (seeNewChangesSinceLastTaskCompletion) {
				...
				const lastTaskCompletedMessageCheckpointHash = findLast(
					this.services.messageStateHandler.getAiHydroMessages().slice(0, messageIndex),
	...
	async doesLatestTaskCompletionHaveNewChanges(): Promise<boolean> {
		try {
			...
			const aihydroMessages = this.services.messageStateHandler.getAiHydroMessages()
			const messageIndex = findLastIndex(aihydroMessages, (m) => m.say === "completion_result")
			const message = aihydroMessages[messageIndex]
			if (!message) {
	...
	private async handleSuccessfulRestore(
		...
		switch (restoreType) {
			...
			case "taskAndWorkspace":
				...
				const aihydroMessages = this.services.messageStateHandler.getAiHydroMessages()
				const deletedMessages = aihydroMessages.slice(messageIndex + 1)
				...
				const newAiHydroMessages = aihydroMessages.slice(0, messageIndex + 1)
				await this.services.messageStateHandler.overwriteAiHydroMessages(newAiHydroMessages) // calls saveAiHydroMessages which saves historyItem
		...
		try {
			checkpointsWarningTimer = setTimeout(async () => {
				if (!checkpointsWarningShown) {
					...
					await this.setcheckpointManagerErrorMessage(
						"Checkpoints are taking longer than expected to initialize. Working in a large repository? Consider re-opening AI-Hydro in a project that uses git, or disabling checkpoints.",
					)
				}
			}, 7_000)
			...
			const tracker = await pTimeout(
				CheckpointTracker.create(this.task.taskId, this.config.enableCheckpoints, workspacePath),
				{
					milliseconds: 15_000,
					message:
						"Checkpoints taking too long to initialize. Consider re-opening AI-Hydro in a project that uses git, or disabling checkpoints.",
				},
			)
			...
			if (errorMessage.includes("Checkpoints taking too long to initialize")) {
				await this.setcheckpointManagerErrorMessage(
					"Checkpoints initialization timed out. Consider re-opening AI-Hydro in a project that uses git, or disabling checkpoints.",
				)
			} else {
				await this.setcheckpointManagerErrorMessage(errorMessage)
			}
