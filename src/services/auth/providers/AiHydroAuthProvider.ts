...
import type { AiHydroAuthInfo } from "../AuthService"
import { IAuthProvider } from "./IAuthProvider"

interface AiHydroAuthApiUser {
	subject: string | null
	email: string
	name: string
...
interface AiHydroAuthResponseData {
	/**
	 ...
	 */
	userInfo: AiHydroAuthApiUser
}

export interface AiHydroAuthApiTokenExchangeResponse {
	success: boolean
	data: AiHydroAuthResponseData
}

export interface AiHydroAuthApiTokenRefreshResponse {
	success: boolean
	data: AiHydroAuthResponseData
}

export class AiHydroAuthProvider implements IAuthProvider {
	readonly name = "cline"
	...
	/**
	 * Retrieves AI-Hydro auth info using the stored access token.
	 * @param controller - The controller instance to access stored secrets.
	 * @returns {Promise<AiHydroAuthInfo | null>} A promise that resolves with the auth info or null.
	 */
	async retrieveAiHydroAuthInfo(controller: Controller): Promise<AiHydroAuthInfo | null> {
		try {
			...
			let storedAuthData: AiHydroAuthInfo
			try {
	 ...
	 * @returns {Promise<AiHydroAuthInfo>} The new access token and user info.
	 */
	async refreshToken(refreshToken: string): Promise<AiHydroAuthInfo> {
		try {
			// Get the callback URL that was used during the initial auth request
			const endpoint = new URL(AIHYDRO_API_ENDPOINT.REFRESH_TOKEN, this.config.apiBaseUrl)
			...
			const data: AiHydroAuthApiTokenExchangeResponse = await response.json()
	...
	async getAuthRequest(callbackUrl: string): Promise<string> {
		const authUrl = new URL(AIHYDRO_API_ENDPOINT.AUTH, this.config.apiBaseUrl)
	...
	async signIn(controller: Controller, authorizationCode: string, provider: string): Promise<AiHydroAuthInfo | null> {
		try {
			// Get the callback URL that was used during the initial auth request
			...
			const tokenUrl = new URL(AIHYDRO_API_ENDPOINT.TOKEN_EXCHANGE, this.config.apiBaseUrl)
			...
			const responseType: AiHydroAuthApiTokenExchangeResponse = responseJSON
