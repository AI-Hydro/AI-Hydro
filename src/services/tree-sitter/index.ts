...
export async function parseSourceCodeForDefinitionsTopLevel(
	...
	const allowedFilesToParse = aihydroIgnoreController ? aihydroIgnoreController.filterPaths(filesToParse) : filesToParse

	for (const filePath of allowedFilesToParse) {
		const definitions = await parseFile(filePath, languageParsers, aihydroIgnoreController)
		if (definitions) {
...
async function parseFile(
	...
	if (aihydroIgnoreController && !aihydroIgnoreController.validateAccess(filePath)) {
		return null
	}
