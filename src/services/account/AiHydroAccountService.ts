import type {
	...
	UsageTransaction,
	UserResponse,
} from "@shared/AiHydroAccount"
import axios, { AxiosRequestConfig, AxiosResponse } from "axios"
import { AiHydroEnv } from "@/config"
import { AIHYDRO_API_ENDPOINT } from "@/shared/aihydro/api"
import { AuthService } from "../auth/AuthService"

export class AiHydroAccountService {
	private static instance: AiHydroAccountService
	...
	/**
	 * Returns the singleton instance of AiHydroAccountService
	 * @returns Singleton instance of AiHydroAccountService
	 */
	public static getInstance(): AiHydroAccountService {
		if (!AiHydroAccountService.instance) {
			AiHydroAccountService.instance = new AiHydroAccountService()
		}
		return AiHydroAccountService.instance
	}

	/**
	 * Returns the base URL for the AI-Hydro API
	...
	/**
	 * Helper function to make authenticated requests to the AI-Hydro API
	...
	async fetchMe(): Promise<UserResponse | undefined> {
		try {
			const data = await this.authenticatedRequest<UserResponse>(AIHYDRO_API_ENDPOINT.USER_INFO)
			return data
		} catch (error) {
	...
	async switchAccount(organizationId?: string): Promise<void> {
		...
		try {
			...
			const _response = await this.authenticatedRequest<string>(AIHYDRO_API_ENDPOINT.ACTIVE_ACCOUNT, {
				method: "PUT",
				headers: {
					"Content-Type": "application/json",
				},
	...
	/**
	 * Transcribes audio using the AI-Hydro transcription service
