...
export class AiHydroError extends Error {
	readonly title = "AiHydroError"
	readonly _error: ErrorDetails

	// Error details per providers:
	// AI-Hydro: error?.error
	// Ollama: error?.cause
	// tbc
	constructor(
		raw: any,
	...
	/**
	 * Parses a stringified error into a AiHydroError instance.
	 */
	static parse(errorStr?: string, modelId?: string): AiHydroError | undefined {
		if (!errorStr || typeof errorStr !== "string") {
			return undefined
		}
		return AiHydroError.transform(errorStr, modelId)
	}

	/**
	 * Transforms any object into a AiHydroError instance.
	 * Always returns a AiHydroError, even if the input is not a valid error object.
	 */
	static transform(error: any, modelId?: string, providerId?: string): AiHydroError {
		try {
			// If already a AiHydroError, return it directly to prevent infinite recursion
			if (error instanceof AiHydroError) {
				return error
			}
			return new AiHydroError(JSON.parse(error), modelId, providerId)
		} catch {
			return new AiHydroError(error, modelId, providerId)
		}
	}

	public isErrorType(type: AiHydroErrorType): boolean {
		return AiHydroError.getErrorType(this) === type
	}
	...
	static getErrorType(err: AiHydroError): AiHydroErrorType | undefined {
		...
		if (message) {
			...
			if (RATE_LIMIT_PATTERNS.some((pattern) => pattern.test(lowerMessage))) {
				return AiHydroErrorType.RateLimit
			}
		}
